...
def login(userid, body):
    ...
    return API.OK({ "session": session }, { 'Set-Cookie': 'session={0}; Domain=ha-timetracker.netlify.app; Path=/; Max-Age=3600; SameSite=None; Secure'.format(session) })
...
def logout(userid, body):
    ...
    return API.OK(None, { 'Set-Cookie': 'session=; Path=/; Max-Age=-1; Domain=ha-timetracker.netlify.app; SameSite=None; Secure'})